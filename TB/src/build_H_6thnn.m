...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Hmat,gradH_x] = build_H_6thnn(Hmat,orbitals,Mxz,k,nlayer,multilayer,...
...
for layer = 1 : nlayer
    ...
    for ii = 1 : size(liorb,2)
        ...
        % Contribution from 1st-nearest neighbors
        nn1 = size(orbitals(iorb).Intra_nn1_list,2);
        for in = 1 : nn1
            jorb = orbitals(iorb).Intra_nn1_list(in);
            hopping = orbitals(iorb).Intra_nn1_hoppings(in);
            delta = orbitals(iorb).Intra_nn1_centre(in,1:3) - orbitals(iorb).Centre;
        ...
        % Contribution from 2nd-nearest neighbors
        nn2 = size(orbitals(iorb).Intra_nn2_list,2);
        for in = 1 : nn2
            jorb = orbitals(iorb).Intra_nn2_list(in);
            hopping = orbitals(iorb).Intra_nn2_hoppings(in);
            delta = orbitals(iorb).Intra_nn2_centre(in,1:3) - orbitals(iorb).Centre;                       hij = hopping*exp(1i*dot(delta,k));
            ...
            elseif(orbitals(jorb).Rel_index == orbitals(iorb).Rel_index)
                Hmat(iorb,jorb) = Hmat(iorb,jorb) + hij;
                if(gradient)
                    gradH_x(iorb,jorb) = gradH_x(iorb,jorb) - delta(1)*hij - 1i*delta(2)*hij;
                end
        ...
        % Contribution from 3rd-nearest neighbors
        nn3 = size(orbitals(iorb).Intra_nn3_list,2);
        for in = 1 : nn3
            jorb = orbitals(iorb).Intra_nn3_list(in);
            hopping = orbitals(iorb).Intra_nn3_hoppings(in);
            delta = orbitals(iorb).Intra_nn3_centre(in,1:3) - orbitals(iorb).Centre;
        ...
        % Contribution from 4th-nearest neighbors
        nn4 = size(orbitals(iorb).Intra_nn4_list,2);
        for in = 1 : nn4
            jorb = orbitals(iorb).Intra_nn4_list(in);
            hopping = orbitals(iorb).Intra_nn4_hoppings(in);
            delta = orbitals(iorb).Intra_nn4_centre(in,1:3) - orbitals(iorb).Centre;
        ...
        % Contribution from 5th-nearest neighbors
        nn5 = size(orbitals(iorb).Intra_nn5_list,2);
        for in = 1 : nn5
            jorb = orbitals(iorb).Intra_nn5_list(in);
            hopping = orbitals(iorb).Intra_nn5_hoppings(in);
            delta = orbitals(iorb).Intra_nn5_centre(in,1:3) - orbitals(iorb).Centre;
            hij = hopping*exp(1i*dot(delta,k));
            if(orbitals(jorb).Rel_index > orbitals(iorb).Rel_index)
                Hmat(iorb,jorb) = Hmat(iorb,jorb) + hij;
                Hmat(jorb,iorb) = Hmat(jorb,iorb) + conj(hij); %hopping*exp(1i*dot(delta,k));
        ...
        % Contribution from 6th-nearest neighbors
        nn6 = size(orbitals(iorb).Intra_nn6_list,2);
        for in = 1 : nn6
            jorb = orbitals(iorb).Intra_nn6_list(in);
            hopping = orbitals(iorb).Intra_nn6_hoppings(in);
            delta = orbitals(iorb).Intra_nn6_centre(in,1:3) - orbitals(iorb).Centre;
            hij = hopping*exp(1i*dot(delta,k));
            if(orbitals(jorb).Rel_index > orbitals(iorb).Rel_index)
                Hmat(iorb,jorb) = Hmat(iorb,jorb) + hij;
                Hmat(jorb,iorb) = Hmat(jorb,iorb) + conj(hij); %hopping*exp(1i*dot(delta,k));
...
%   disp('WARNING: Hamiltonian is not Hermitian!')
%   error('Error: Hamiltonian is not hermitian!')
%end
