...
function [Hmat,tb_bands,tb_vecs,gradH_x] = opt_build_and_diag_H_6thnn(task,Hmat,...
...
if(~read_ham)
   ...
   for layer = 1 : nlayer
	  ...
	  sum_nnz = sum_nnz + length(orb1.Intra_nn1_list) + length(orb1.Intra_nn2_list) + length(orb1.Intra_nn3_list) + length(orb1.Intra_nn4_list) + length(orb1.Intra_nn5_list) + length(orb1.Intra_nn6_list) + length(orb1.Inter_nn1_list) + length(orb1.Inter_nn2_list) + length(orb1.Inter_mx_n_list) + 1;
	 if(orb1.Rel_index ==  7 || orb1.Rel_index == 8)
	    dr = -orb1.dr*g1;
         %elseif(orb1.Rel_index == 3 || orb1.Rel_index == 9) %orb1.Rel_index == 6 ||
         %   dr = -orb1.dr*gpz;
	 else
	    dr = 0.0;	   
         end  
          diagH(orb1.Ham_index) = diagH(orb1.Ham_index) + onsite(orb1.Rel_index,layer) + dr;
	  ...
	  if(ef_strength ~= 0.0)
	     ...
	     elseif(orb1.l == 1)
	        z_coor = dot(orb1.Pcentre,[0 0 1]);
             ...
             diagH(orb1.Ham_index) = diagH(orb1.Ham_index) + ef_strength*z_coor;
         end
      end
   ...
   if(reduced_workspace)
	   [i,j,s] = find(Mxz);
	   Mxz = sparse(i,j,s,tot_norbs,tot_norbs);
           %Mxz = sparse(Mxz);
   end
   ...
   spmd
      ...
      for loc_ik = 1 : length(globalInd)
        ik = globalInd(loc_ik);
        if(reduced_workspace)
	   H = spalloc(tot_norbs,tot_norbs,sum_nnz);
	   if(gradient)
	     gH = spalloc(tot_norbs,tot_norbs,sum_nnz);
	   end
           H = H + sparse(diag(diagH));
        else
          H = diag(diagH);
          gH = zeros(tot_norbs);
        end
